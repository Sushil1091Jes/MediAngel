...
(function () {
    ...
    ProvidersHelper.prototype.bindEventHandlers = function () {
        ...
        $('#providers').on('click', '#save-provider', function () {
            ...
            //upload image ,save
            this.uploadImageAndSave(provider);
    ...
    ProvidersHelper.prototype.validate = function () {
        ...
        try {
            ...
            // Check if headPic 
            if ($('#upload_image')[0].files[0] == undefined && ($('#provider-headPic').val() === '' || $('#provider-headPic').val() === null)) {
                throw new Error(EALang.headPic_upload);
            }
    ...
    ProvidersHelper.prototype.display = function (provider) {
        ...
        $('#provider-headPic').val('');
        $('#uploaded_image').attr("src", './../../assets/img/user.png');
        if (provider.headPic) {
            $('#provider-headPic').val(provider.headPic);
            $('#uploaded_image').attr("src", provider.headPic);
        }
    ...
    ProvidersHelper.prototype.uploadImageAndSave = function (provider) {
        ...
        try {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.file == "") {
                        throw new Error(EALang.headPic_upload_error);
                    } else {
                        $("#provider-headPic").val(data.file);
                        $('#upload_image').val('');
                        provider.headPic = data.file;
                        that.save(provider);
                    }
                }
            });
