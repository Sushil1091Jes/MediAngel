...
class Google extends EA_Controller {
     ...
     * documentation of OAuth), every MediAngel installation should use its own calendar api key. So in every
     * api console account, the "http://path-to-MediAngel/google/oauth_callback" should be included in an allowed redirect URL.
     */
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and MediAngel.
     *
    ...
    public static function sync($provider_id = NULL)
    {
        try
        {
            ...
            foreach ($appointments as $appointment)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new Exception('Event is cancelled, remove the record from MediAngel.');
                        }

                        // If Google Calendar event is different from MediAngel appointment then update
                        // MediAngel record.
                        $is_different = FALSE;
            ...
            // Add Google Calendar events that do not exist in MediAngel.
            ...
            foreach ($google_events->getItems() as $google_event)
            {
                ...
                // Record doesn't exist in the MediAngel, so add the event now.
                $appointment = [
