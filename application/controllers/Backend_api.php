...
class Backend_api extends EA_Controller {
     ...
     * Cancel appointment.
     *
     * This method cancel an existing appointment from the database. Once this action is finished it cannot be undone.
     * Notification emails are send to both provider and customer and the delete action is executed to the Google
     * Calendar account of the provider, if the "google_sync" setting is enabled.
     */
    public function ajax_cancel_appointment()
    {
        try
        {
            ...
            $cancel_appointment = $this->appointments_model->get_row($this->input->post('appointment_id'));
            ...
            */
            ...
            // Update appointment record set to canceled from the database.
            $cancel_appointment['status']=-1;
            $this->appointments_model->add($cancel_appointment);

            // Sync removal with Google Calendar.
            if ($cancel_appointment['id_google_calendar'] != NULL)
            {
                try
                {
                    $google_sync = $this->providers_model->get_setting('google_sync', $provider['id']);
                    ...
                    if ($google_sync == TRUE)
                    {
                        ...
                        $this->load->library('Google_sync');
                        $this->google_sync->refresh_token($google_token->refresh_token);
                        $this->google_sync->delete_appointment($provider, $cancel_appointment['id_google_calendar']);
                    }
                }
                catch (Exception $exception)
                {
            ...
            try
            {
                ...
                if ((bool)$send_provider === TRUE)
                {
                    $email->send_delete_appointment($cancel_appointment, $provider,
                        $service, $customer, $settings, new Email($provider['email']),
                        new Text($this->input->post('delete_reason')));
                }
    ...
    /**
     * Save (insert or update) category template record.
     */
    public function ajax_save_service_category_template()
    {
        try
        {
            $this->load->model('categories_template_model');
            $category_template = json_decode($this->input->post('category_template'), TRUE);

            $required_privileges = ( ! isset($category_template['id']))
                ? $this->privileges[PRIV_SERVICES]['add']
                : $this->privileges[PRIV_SERVICES]['edit'];
            if ($required_privileges == FALSE)
            {
            ...
            $category_template_id = $this->categories_template_model->add($category_template);
    ...
    /**
     * Delete category template record from database.
     */
    public function ajax_delete_service_category_template()
    {
        try
        {
            if ($this->privileges[PRIV_SERVICES]['delete'] == FALSE)
            ...
            $result = $this->categories_template_model->delete($this->input->post('category_template_id'));
    ...
    /**
     * Filter category template with key string.
     */
    public function ajax_filter_service_categories_template()
    {
        try
        {
            if ($this->privileges[PRIV_SERVICES]['view'] == FALSE)
            ...
            $response = $this->categories_template_model->get_batch($where);
        }
        catch (Exception $exception)
        {
            $this->output->set_status_header(500);
