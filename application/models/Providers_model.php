 ...
 *
 * Contains the database operations for the service provider users of MediAngel.
 *
 * @package Models
 */
class Providers_model extends EA_Model {
    ...
    public function validate($provider)
    {
        ...
        $num_rows = $this->db
            ...
            ->where('users.last_name', $provider['last_name'])
        ...
        if ($num_rows > 0)
        {
            throw new Exception('Given info belongs to another provider record. '
                . 'Please use a different email,first_name,last_name.');
        }
    ...
    public function get_providers_serviceName($service_name)
    {
		$this->db
            ->select('ea_users.* ,
                      ea_services.id as services_id,ea_services.name as services_name, ')
            ->from('ea_services')
            ->join('ea_services_providers', 'ea_services.id = ea_services_providers.id_services', 'inner')
			->join('ea_users', 'ea_users.id = ea_services_providers.id_users', 'inner')
            ->join('ea_roles', 'ea_roles.id = ea_users.id_roles', 'inner')
			->where('ea_services.name', $service_name)
    ...
    public function get_providers_service_by($service_id)
    {
		  $sql="select distinct t1.*  
                from  (select distinct providers.*, 
                            services.id as service_id, services.name  as service_name ,
	                        (case when categories.id is null then 0 else services.id_service_categories end) as category_id,
                            categories.name as category_name
                        from ea_users as providers
                        join ea_services_providers as services_providers on providers.id=services_providers.id_users
                        join ea_services as services on services_providers.id_services= services.id
                        left join ea_service_categories as categories on services.id_service_categories=categories.id) as t1
                  join (select distinct services_providers.id_users as user_id ,
							  services.id as service_id,
								(case when services.id_service_categories is null then 0 else services.id_service_categories end) as category_id
			             from ea_services as services
			             join ea_services_providers as services_providers  
					     on services.id= services_providers.id_services)  as t2  
                   on 	t1.id=t2.user_id  
