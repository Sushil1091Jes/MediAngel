...
class Appointments_model extends EA_Model {
    ...
    public function get_row($appointment_id)
    {
        ...
        $appointment = $this->db->get_where('appointments', ['id' => $appointment_id,'status'=>1])->row_array();
    ...
    public function get_attendants_number_for_period(DateTime $slot_start, DateTime $slot_end, $service_id)
    {
 	...
 	//Generate grouped data for all dates between two dates 
	public function generate_group_data_for_period($start,$end)
	{
		...
		{
			$result[]=['AppointmentDate'=>date('Y-m-d',$dt_start),'Number'=>'0'];
			$dt_start = strtotime('+1 day',$dt_start);
		}
		return $result;
	}
	
	//Generate grouped data by months for all last year
	public function generate_group_data_for_last_year()
	{
		$now = time();
        $old_time = strtotime('-11 month',$now);
        
		for($i = 0;$i < 12; $i++)
		{
			$tempYearMonth = strtotime("+$i month",$old_time);
            $result[]=['YearMonth'=>date('Y-m',$tempYearMonth),'Number'=>'0'];
        }
    ...
    {		
        $query="SELECT A.MedicalField,CONCAT_WS(' ',User.first_name,User.last_name) AS Doctor,A.AppointmentDate,A.Patient,A.Email,A.Mobile,A.Phone,A.Hash 
                FROM 
                    (SELECT SVR.name AS MedicalField,AP.start_datetime AS AppointmentDate,CONCAT_WS(' ',U.first_name,U.last_name) AS Patient,IFNULL(U.email,'') AS Email
						,IFNULL(U.mobile_number,'') AS Mobile,IFNULL(U.phone_number,'') AS Phone,AP.hash AS Hash,AP.id_users_provider
                    FROM ea_appointments AP
                        INNER JOIN ea_users U ON U.id=AP.id_users_customer
                        INNER JOIN ea_services SVR ON SVR.id=AP.id_services
                    WHERE AP.status=1
                        AND AP.start_datetime>NOW()
                        AND U.email='{$email}') A
                    INNER JOIN ea_users User ON User.id=A.id_users_provider
                ORDER BY A.AppointmentDate";
        return $this->db->query($query)->result_array();
    }

    public function get_appointments_number_for_period($slot_start, $slot_end)
    {		
        return (int)$this->db
            ->select('count(*) AS appointments_number')
            ->from('appointments')
            ->where('start_datetime >=', date('Y-m-d 00:00:00',strtotime($slot_start)))
            ->where('end_datetime <=', date('Y-m-d 23:59:59',strtotime($slot_end)))
            ->where('status=',1)
            ->get()
            ->row()
            ->appointments_number;
	...
	public function get_appointments_group_number_for_period($slot_start, $slot_end)
    {		
		$query="SELECT CAST(start_datetime as date) AS AppointmentDate, COUNT(*) AS Number FROM ea_appointments 
					WHERE status=1
                        AND CAST(start_datetime as date)>='{$slot_start}' 
						AND CAST(end_datetime as date)<='{$slot_end}'						  
					GROUP by CAST(start_datetime as date) ORDER by CAST(start_datetime as date)";
        return $this->db->query($query)->result_array();
    }
	
	public function get_newpatients_number_for_period($slot_start, $slot_end)
    {		
		$query="SELECT COUNT(*) AS NewPatients FROM ea_appointments 
					WHERE status=1 
                        AND CAST(start_datetime as date)>='{$slot_start}' 
                        AND CAST(end_datetime as date)<='{$slot_end}' 
                        AND id_users_customer not in
						(SELECT id_users_customer FROM ea_appointments WHERE status=1 AND CAST(start_datetime as date)<'{$slot_start}')";
        return $this->db->query($query)->row()->NewPatients;
    }
	
	public function get_newpatients_group_number_for_period($slot_start, $slot_end)
    {
		$query="SELECT CAST(start_datetime as date) AS AppointmentDate,COUNT(*) AS Number FROM ea_appointments
					WHERE status=1
                        AND CAST(start_datetime as date)>='{$slot_start}' 
	...
	public function get_todayappointments_number()
    {
		$query="SELECT COUNT(*) AS TodayAppointments 
				FROM ea_appointments
				WHERE status=1 AND CAST(start_datetime as date)=CURDATE()";
        return $this->db->query($query)->row()->TodayAppointments;
    }
	
	public function get_group_total_patients_number_for_last_year()
    {
		$query="SELECT DATE_FORMAT(start_datetime,'%Y-%m') AS YearMonth,COUNT(DISTINCT U.id) AS Number
				FROM ea_appointments AP
					INNER JOIN ea_users U ON U.id=AP.id_users_customer
				WHERE AP.status=1 
                    AND DATE_FORMAT(start_datetime,'%Y-%m-%d')>DATE_SUB(LAST_DAY(CURDATE()),INTERVAL 1 YEAR)
                    AND DATE_FORMAT(start_datetime,'%Y-%m-%d')<= LAST_DAY(CURDATE())
				GROUP BY DATE_FORMAT(start_datetime,'%Y-%m')";
        return $this->db->query($query)->result_array();
    }
	
	public function get_group_new_patients_number_for_last_year()
    {
		$query="SELECT DATE_FORMAT(start_datetime,'%Y-%m') AS YearMonth,COUNT(DISTINCT U.id) AS Number
				FROM ea_appointments AP
					INNER JOIN ea_users U ON U.id=AP.id_users_customer
				WHERE AP.status=1
                    ...
                    AND DATE_FORMAT(start_datetime,'%Y-%m-%d')<= LAST_DAY(CURDATE())
                    AND id_users_customer NOT IN
                        (SELECT id_users_customer FROM ea_appointments WHERE status=1 AND DATE_FORMAT(start_datetime,'%Y-%m-%d')<=DATE_SUB(LAST_DAY(CURDATE()),INTERVAL 1 YEAR))
				GROUP BY DATE_FORMAT(start_datetime,'%Y-%m')";
        return $this->db->query($query)->result_array();
    }
	
	public function get_group_repeat_patients_number_for_last_year()
    {
		$query="SELECT DATE_FORMAT(start_datetime,'%Y-%m') AS YearMonth,COUNT(DISTINCT U.id) AS Number
				FROM ea_appointments AP
					INNER JOIN ea_users U ON U.id=AP.id_users_customer
	...
	public function get_latest_appointments_user()
    {
		$query="SELECT AP.start_datetime AS AppointmentDate,U.first_name AS FirstName,U.last_name AS LastName,IFNULL(U.email,'') AS Email
						,IFNULL(U.mobile_number,'') AS Mobile,IFNULL(U.phone_number,'') AS Phone,IFNULL(U.city,'') AS City,IFNULL(U.address,'') AS Address
						,U.timezone AS TimeZone,IFNULL(U.zip_code,'') AS Zip
				FROM ea_appointments AP
					INNER join ea_users U ON U.id=AP.id_users_customer
				WHERE AP.status=1
                    AND DATE_FORMAT(AP.start_datetime,'%Y-%m-%d')=DATE_FORMAT(CURDATE(),'%Y-%m-%d') 
                    AND AP.start_datetime>NOW()
				ORDER BY AP.start_datetime LIMIT 0,1";
        return $this->db->query($query)->result_array();
    }
	
	
	public function get_group_number_patients_by_gender_for_period($slot_start, $slot_end)
    {
		$query="SELECT U.gender AS GenderType,COUNT(DISTINCT U.id) AS Number
				FROM ea_appointments AP
					INNER JOIN ea_users U ON U.id=AP.id_users_customer
				WHERE AP.status=1
                    AND DATE_FORMAT(start_datetime,'%Y-%m-%d')>='{$slot_start}'
                    AND DATE_FORMAT(start_datetime,'%Y-%m-%d')<= '{$slot_end}'
				GROUP BY U.gender";
        return $this->db->query($query)->result_array();
    }

    public function verify_date_is_reserved($provider_id, $start_datetime, $end_datetime)
    {
		$query="select *  from ea_appointments
              where status=1 and id_users_provider={$provider_id} and 
              start_datetime<'{$end_datetime}' and end_datetime >'{$start_datetime}'";
        $num_rows= $this->db->query($query)->num_rows();
        if ($num_rows>0){
             throw new Exception('This time period has been reserved, please re select the reservation time.');
        }
    }
