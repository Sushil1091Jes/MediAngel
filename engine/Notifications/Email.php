...
class Email {
    ...
    )
    {
        ...
        $mailer->From = $this->config['smtp_user'] ;//$company['company_email'];
     ...
     * @throws \PHPMailer\PHPMailer\Exception
     */
    public function send_sms_code(NonEmptyText $verification_code, EmailAddress $recipientEmail, array $company)
    {
        $email_title = $this->framework->lang->line('sms_code_title');
        $verification_code = $verification_code->get();
        $email_message = str_replace('$code', $verification_code, $this->framework->lang->line('sms_code_msg'));
        $company_name = $company['company_name'];
        $company_email = $company['company_email'];
        $company_link = $company['company_link'];

        ob_start();
        require __DIR__ . '/../../application/views/emails/sms_code.php';
        $html = ob_get_clean();
        ...
        $mailer->FromName = $company['company_name'];
        $mailer->AddAddress($recipientEmail->get()); // "Name" argument crushes the phpmailer class.
        $mailer->Subject = $this->framework->lang->line('sms_code_title');
        $mailer->Body = $html;
